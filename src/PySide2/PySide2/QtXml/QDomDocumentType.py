# encoding: utf-8
# module PySide2.QtXml
# from D:\Maya2024\Python\lib\site-packages\PySide2\QtXml.cp310-win_amd64.pyd
# by generator 1.147
# no doc

# imports
import Shiboken as __Shiboken


from .QDomNode import QDomNode

class QDomDocumentType(QDomNode):
    """
    QDomDocumentType(self) -> None
    QDomDocumentType(self, x: PySide2.QtXml.QDomDocumentType) -> None
    """
    def entities(self): # real signature unknown; restored from __doc__
        """ entities(self) -> PySide2.QtXml.QDomNamedNodeMap """
        pass

    def internalSubset(self): # real signature unknown; restored from __doc__
        """ internalSubset(self) -> str """
        return ""

    def name(self): # real signature unknown; restored from __doc__
        """ name(self) -> str """
        return ""

    def nodeType(self): # real signature unknown; restored from __doc__
        """ nodeType(self) -> PySide2.QtXml.QDomNode.NodeType """
        pass

    def notations(self): # real signature unknown; restored from __doc__
        """ notations(self) -> PySide2.QtXml.QDomNamedNodeMap """
        pass

    def publicId(self): # real signature unknown; restored from __doc__
        """ publicId(self) -> str """
        return ""

    def systemId(self): # real signature unknown; restored from __doc__
        """ systemId(self) -> str """
        return ""

    def __bool__(self, *args, **kwargs): # real signature unknown
        """ True if self else False """
        pass

    def __copy__(self): # real signature unknown; restored from __doc__
        """ __copy__() -> None """
        pass

    def __init__(self): # real signature unknown; restored from __doc__
        pass

    @staticmethod # known case of __new__
    def __new__(*args, **kwargs): # real signature unknown
        """ Create and return a new object.  See help(type) for accurate signature. """
        pass


