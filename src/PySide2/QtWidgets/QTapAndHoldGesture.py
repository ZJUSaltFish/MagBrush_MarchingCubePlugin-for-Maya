# encoding: utf-8
# module PySide2.QtWidgets
# from D:\Maya2024\Python\lib\site-packages\PySide2\QtWidgets.cp310-win_amd64.pyd
# by generator 1.147
# no doc

# imports
import PySide2.QtCore as __PySide2_QtCore
import PySide2.QtGui as __PySide2_QtGui
import Shiboken as __Shiboken


from .QGesture import QGesture

class QTapAndHoldGesture(QGesture):
    """ QTapAndHoldGesture(self, parent: typing.Optional[PySide2.QtCore.QObject] = None) -> None """
    def position(self): # real signature unknown; restored from __doc__
        """ position(self) -> PySide2.QtCore.QPointF """
        pass

    def setPosition(self, pos): # real signature unknown; restored from __doc__
        """ setPosition(self, pos: PySide2.QtCore.QPointF) -> None """
        pass

    def setTimeout(self, msecs): # real signature unknown; restored from __doc__
        """ setTimeout(msecs: int) -> None """
        pass

    def timeout(self): # real signature unknown; restored from __doc__
        """ timeout() -> int """
        return 0

    def __delattr__(self, *args, **kwargs): # real signature unknown
        """ Implement delattr(self, name). """
        pass

    def __init__(self, parent, PySide2_QtCore_QObject=None, *args, **kwargs): # real signature unknown; NOTE: unreliably restored from __doc__ 
        pass

    @staticmethod # known case of __new__
    def __new__(*args, **kwargs): # real signature unknown
        """ Create and return a new object.  See help(type) for accurate signature. """
        pass

    def __setattr__(self, *args, **kwargs): # real signature unknown
        """ Implement setattr(self, name, value). """
        pass

    staticMetaObject = None # (!) real value is '<PySide2.QtCore.QMetaObject object at 0x0000014F5085BF80>'


